...
const longPeriod = 21;
const rsiPeriod = 14;
const lastSignals = {}; // track last signal per pair
...
ws.on("message", msg => {
  ...
  if (candles[pair].length > 500) candles[pair].shift();
...
function checkSignal(pair) {
  ...
  if (closes.length < longPeriod + 2) return;
  ...
  const now = Date.now();
  if (lastSignals[pair] && now - lastSignals[pair] < 3600 * 1000) return;
...
// --- Send Signal + Trade ---
async function sendSignal(pair, side, entry, now) {
  ...
  // Place trade
  try {
    await placeOrder(pair, side === "LONG" ? "Buy" : "Sell", entry, sl, tp, 10); // 10% balance
  } catch (err) {
